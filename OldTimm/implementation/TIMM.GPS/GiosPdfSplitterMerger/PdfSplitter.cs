...
namespace Gios.Pdf.SplitMerge
{
    ...
    {
        internal Dictionary<int, PdfFileObject> sObjects;
        internal List<int> pageNumbers;
        internal Dictionary<int, int> transHash;
		...
		internal void Load(PdfFile PdfFile,int[] PageNumbers, int startNumber)
		{
			...
			this.sObjects = new Dictionary<int, PdfFileObject>();
			int part=0;
        ...
        private Dictionary<int,int> CalcTransHash(int startNumber)
        {
            Dictionary<int, int> ht = new Dictionary<int, int>();
            List<PdfFileObject> al = new List<PdfFileObject>();
            foreach (PdfFileObject pfo in this.sObjects.Values)
            {
                al.Add(pfo);
            }
