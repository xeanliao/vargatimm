...
namespace Gios.Pdf.SplitMerge
{
    internal class PdfFileObject
    {
        ...
        internal void PopulateRelatedObjects(PdfFile PdfFile, Dictionary<int, PdfFileObject> container)
        {
            if (!container.ContainsKey(this.number))
            {
                ...
                while (m.Success)
                {
                    ...
                    if (notparent & !container.ContainsKey(num))
                    {
                        PdfFileObject pfo = PdfFile.LoadObject(num);
                        if (pfo != null & !container.ContainsKey(pfo.number))
                        {
        ...
        private Dictionary<int, int> TransformationHash;
        private string FilterEval(Match m)
        {
            ...
            if (this.TransformationHash.ContainsKey(id))
            {
                ...
                return TransformationHash[id] + rest+end;
            }
            return end;            
        }
        ...
        internal virtual void Transform(Dictionary<int, int> TransformationHash)
        {
        ...
        internal virtual long WriteToStream(Stream Stream)
        {
            ...
            StreamWriter sw = new StreamWriter(Stream, new ASCIIEncoding());
            sw.Write(this.text);
            sw.Flush();
    ...
    internal class PdfFileObjectNumberComparer : IComparer<PdfFileObject>
    {
        #region IComparer Members

        public int Compare(PdfFileObject x, PdfFileObject y)
        {
