...
namespace ImageTools.IO.Png
{
    ...
    public class PngEncoder
    {
        ...
        /// </exception>
        public void Encode(byte[] data, int pixelWidth, int pixelHeight, Stream stream)
        {
            _stream = stream;
            _image = data;
        ...
        private void WritePhysicsChunk()
        {
            ...
            {
                int dpmX = (int)Math.Round(densityX * 39.3700787d);
                int dpmY = (int)Math.Round(densityY * 39.3700787d);
        ...
        private void WriteDataChunksFast()
        {
            ...
            // the filter byte.
            // --------------------------------------------------
            byte[] data = new byte[_pixelWidth * _pixelHeight * 4 + _pixelHeight];

            int rowLength = _pixelWidth * 4 + 1;

            for (int y = 0; y < _pixelHeight; y++)
            {
                data[y * rowLength] = 0;

                Array.Copy(pixels, y * _pixelWidth * 4, data, y * rowLength + 1, _pixelWidth * 4);
            }
            // --------------------------------------------------
        ...
        private void WriteDataChunks()
        {
            ...
            for (int y = 0; y < _pixelHeight; y++)
            {
                ...
                for (int x = 0; x < _pixelWidth; x++)
                {
                    ...
                    int pixelOffset = (y * _pixelWidth + x) * 4;
                    ...
                    if (y > 0)
                    {
                        int lastOffset = ((y - 1) * _pixelWidth + x) * 4;
