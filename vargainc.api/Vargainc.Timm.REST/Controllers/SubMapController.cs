...
namespace Vargainc.Timm.REST.Controllers
{
    ...
    public class SubMapController : BaseController
    {
        ...
        public async Task<IHttpActionResult> MergeAreas(int campaignId, int submapId, [FromBody] List<AreaRecord> records)
        {
            ...
            List<SubMapHole> holes = new List<SubMapHole>();
            ...
            while (sqlReader.Read())
            {
                ...
                if (!existsArea.Contains(id))
                {
                    ...
                    holes.Add(new SubMapHole { SubmapId = submapId, AreaId = id, Code = code, Apt = holeApt, Home = holeHome });
                }
            }
            sqlReader.Close();

            await db.SubMapHoles.Where(i => i.SubmapId == submapId).DeleteAsync().ConfigureAwait(false);

            if (holes.Count > 0)
            {
                db.SubMapHoles.AddRange(holes);
                await db.SaveChangesAsync().ConfigureAwait(false);
            }
