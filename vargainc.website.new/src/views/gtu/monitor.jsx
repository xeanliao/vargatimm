...
import $ from 'jquery'
import { map, each, xor, isEmpty, indexOf, filter, keys, difference, clone, pull, isInteger } from 'lodash'
...
var reloadTimeout = null,
    ...
    angleIcon =
        'M0.9,0C0.9,0,0.9,0,0.9,0l-1.4-1.5c0,0,0,0-0.1,0c0,0,0,0-0.1,0l-0.2,0.2c0,0,0,0,0,0.1s0,0,0,0.1L0.3,0l-1.2,1.2c0,0,0,0,0,0.1c0,0,0,0,0,0.1l0.2,0.2c0,0,0,0,0.1,0c0,0,0,0,0.1,0L0.9,0C0.9,0,0.9,0,0.9,0z'
...
export default React.createBackboneClass({
    ...
    componentDidMount: function () {
        ...
        this.drawDmapBoundary().done(function () {
            ...
            var allAssignedGtu = map(
                self.props.gtu.where(function (i) {
    ...
    shouldComponentUpdate: function (nextProps, nextState) {
        var oldActiveGtu = nextState.activeGtu,
            ...
            difference = xor(oldActiveGtu, newActiveGtu)
    ...
    drawGtu: function () {
        ...
        each(gtuPoints, function (p) {
            p.setMap(null)
        })
        gtuPoints = []

        each(dots, function (gtu) {
            if (indexOf(activeGtu, gtu.gtuId) == -1) {
                return true
            }
            if (needFilterOutOfBoundary) {
                var points = filter(gtu.points, {
                    out: false,
                })
            } else {
    ...
    animateDrawTrack: function (gtu) {
        ...
        if (animateSerial % 10 == 0) {
            ...
            animateSerial++
            trackAnimationFrame = window.requestAnimationFrame(
                $.proxy(function () {
    ...
    drawLastLocation: function () {
        var googleMap = this.getGoogleMap(),
            ...
            willDrawGtu = map(gtuList, function (gtu) {
                return gtu.get('Id')
            }),
            drawedGtu = keys(gtuLocation) || [],
            needDeleteGtu = difference(drawedGtu, willDrawGtu)

        each(needDeleteGtu, function (id) {
            gtuLocation[id].setMap(null)
            delete gtuLocation[id]
        })

        each(gtuList, function (gtu) {
            var gtuId = gtu.get('Id'),
    ...
    clearMap: function () {
        try {
            ...
            animateIndex = 0
            each(gtuTrack, function (item) {
                item &&
                    each(item, function (point) {
    ...
    onSelectedGTU: function (gtuId) {
        var activeGtu = clone(this.state.activeGtu)
        if (this.state.displayMode == 'cover') {
            if (indexOf(activeGtu, gtuId) > -1) {
                pull(activeGtu, gtuId)
            } else {
    ...
    renderMoreMenu: function () {
        ...
        if (!isInteger(this.props.task.get('Id'))) {
            return (
                <span className="float-right">
                    <button className="button cirle" data-toggle="monitor-more-menu" onClick={this.onOpenMoreMenu}>
                        <i className="fa fa-ellipsis-h"></i>
                    </button>
                    <div id="monitor-more-menu" className="dropdown-pane bottom" data-dropdown data-close-on-click="true" data-auto-focus="false" onClick={this.onCloseMoreMenu}>
            ...
            case 0: //started
                return (
                    <div>
                        ...
                        <button className="button" onClick={this.onStop}>
                ...
                <div className="section row gtu-monitor">
                    <div className="small-12 columns">
                        <div className="section-header">
                            <div className="small-12 medium-5 large-3 columns">{this.renderController()}</div>
