...
import { groupBy, map, each, find, filter, indexOf, xor, extend, startsWith, keys, head, last, concat, clone, get, set, trim, isFunction, includes } from 'lodash'
...
var MapContainer = React.createBackboneClass({
    ...
    drawBoundary: function () {
        ...
        var drawDMapPromise = function () {
            return Promise.all(
                ...
                monitorMap.addLayer({
                    ...
                    type: 'fill',
                    filter: ['all', ['==', '$type', 'Polygon'], ['==', 'IsFinished', true]],
                ...
                monitorMap.addLayer({
                    ...
                    type: 'fill',
                    filter: ['all', ['==', '$type', 'Polygon'], ['==', 'IsFinished', false]],
                    source: `boundary-dmap`,
                    paint: {
                        'fill-color': {
                            property: 'userColor',
        ...
        return Promise.all([drawSubmapPromise(), drawDMapPromise()]).then(() => {
    ...
    setDMapFilter: function () {
        ...
        if (!this.state.showAllDMap && activeTaskId) {
            monitorMap.setFilter('boundary-dmap-finished-layer', ['all', ['==', '$type', 'Polygon'], ['==', 'IsFinished', true], ['==', 'TaskId', activeTaskId]])
            monitorMap.setFilter('boundary-dmap-layer', ['all', ['==', '$type', 'Polygon'], ['==', 'IsFinished', false], ['==', 'TaskId', activeTaskId]])
            monitorMap.setFilter('boundary-dmap-line-layer', ['all', ['==', '$type', 'Polygon'], ['==', 'TaskId', activeTaskId]])
            monitorMap.setFilter('boundary-dmap-label-layer', ['all', ['==', '$type', 'Point'], ['==', 'TaskId', activeTaskId]])
            monitorMap.setLayoutProperty('boundary-submap-layer', 'visibility', 'none')
        } else {
            monitorMap.setFilter('boundary-dmap-finished-layer', ['all', ['==', '$type', 'Polygon'], ['==', 'IsFinished', true]])
            monitorMap.setFilter('boundary-dmap-layer', ['all', ['==', '$type', 'Polygon'], ['==', 'IsFinished', false]])
            monitorMap.setFilter('boundary-dmap-line-layer', ['all', ['==', '$type', 'Polygon']])
            monitorMap.setFilter('boundary-dmap-label-layer', ['all', ['==', '$type', 'Point']])
        ...
        if (displayTasks.length > 1) {
            monitorMap.setFilter('boundary-dmap-finished-layer', ['all', ['==', '$type', 'Polygon'], ['==', 'IsFinished', true], concat(['!in', 'TaskId'], displayTasks)])
            each(['boundary-dmap-merge-source-layer', 'boundary-dmap-merge-source-fill-layer'], (layer) => {
                monitorMap.setFilter(layer, ['all', ['==', '$type', 'Polygon'], ['==', 'TaskId', displayTasks[0]]])
            ...
            each(['boundary-dmap-merge-target-layer', 'boundary-dmap-merge-target-fill-layer'], (layer) => {
                monitorMap.setFilter(layer, ['all', ['==', '$type', 'Polygon'], ['==', 'TaskId', displayTasks[1]]])
                monitorMap.setLayoutProperty(layer, 'visibility', 'visible')
            })
        } else {
            monitorMap.setFilter('boundary-dmap-finished-layer', ['all', ['==', '$type', 'Polygon'], ['==', 'IsFinished', true]])
            each(
                ['boundary-dmap-merge-source-layer', 'boundary-dmap-merge-source-fill-layer', 'boundary-dmap-merge-target-layer', 'boundary-dmap-merge-target-fill-layer'],
                (layer) => {
            ...
            if (self.state.tasks.length == 0) {
                mergeTaskButton = (
                    <div className="small-7 small-centered columns">
                        <div className="button-group stacked-for-small" style={{ marginTop: '20px' }}>
                            <button id={task.Id} className="button btnMergeTask">
                                Merge From Here
                            </button>
                        </div>
                    </div>
                ...
                if (includes(mergeTaskIdArray, task.Id)) {
                    mergeTaskButton = (
                        <div className="small-7 small-centered columns">
                            <div className="button-group stacked-for-small" style={{ marginTop: '20px' }}>
                                <button id={task.Id} className="button btnAbortMergeTask">
                                    Abort Merge
                                </button>
                            </div>
                        </div>
                    ...
                    mergeTaskButton = (
                        <div className="small-10 small-centered columns">
                            <div className="button-group stacked-for-small" style={{ marginTop: '20px' }}>
                                ...
                                <button id={task.Id} className="button btnMergeTaskDone">
                                    Confirm Merge
                                </button>
                            </div>
                        </div>
            ...
            setActiveButton = (
                <div className="small-8 align-center columns">
                    <div className="button-group" style={{ marginTop: '20px' }}>
                        <button id={task.Id} className="button btnSetActiveTask">
                            GO
                        </button>
                    </div>
                </div>
    ...
    onTaskAreaClickHandler: function (e) {
        var monitorMap = this.state.map
        var feature = head(
            monitorMap.queryRenderedFeatures(e.point, {
                layers: ['boundary-dmap-layer', 'boundary-dmap-finished-layer', 'boundary-dmap-merge-source-fill-layer', 'boundary-dmap-merge-target-fill-layer'],
            })
        )
        if (feature) {
    ...
    flyToTask: function (taskId, keepZoom) {
        ...
        var feature = head(
            filter(taskSource._data.features, (o) => {
...
export default React.createBackboneClass({
    ...
    loadTask: function (task) {
        ...
        var locationQuery = gtu
            .fetchGtuWithStatusByTask(task.get('Id'), {
                quite: false,
            })
            .then(() => {
    ...
    onMergeTask: function () {
        var self = this
        return Promise.all(
            map(this.state.mergeTasks, (task) => {
    ...
    renderTaskDropdown: function () {
        ...
        if (tasks.length > 10) {
            ...
            return (
                <select ref={this.initSelect2} data-placeholder="Select an task">
                    ...
                    {map(tasks, (t) => {
                        return (
                            <option key={`task-ddl-option-${t.get('Id')}`} value={t.get('Id')}>
                                {t.get('Name')}
                            </option>
                        )
                    })}
        ...
        return (
            <ul className="dropdown menu float-right">
                <li className={parentClass}>
                    <a href="javascript:;" onClick={this.onOpenDropDown}>
                        Switch Active Task
                    </a>
                ...
                <div className="section row gtu-monitor expanded">
                    <div className="small-12 columns">
                        <div className="row expanded">
                            <div className="small-12 medium-5 large-3 columns">{this.renderTaskController(activeTask)}</div>
    ...
    renderTaskController: function (task) {
        ...
        switch (task.get('Status')) {
            case 0: //started
                return (
                    <div>
                        <button className="button" onClick={this.onPause.bind(this, task)}>
                            <i className="fa fa-pause"></i>Pause
                        </button>
                        <button className="button" onClick={this.onStop.bind(this, task)}>
                            <i className="fa fa-stop"></i>Stop
                        </button>
                        <button className={finishClass} onClick={this.onFinished.bind(this, task.get('Id'))}>
            ...
            case 2: //peased
                return (
                    <div>
                        <button className="button" onClick={this.onStart.bind(this, task)}>
            ...
            default:
                return (
                    <div>
                        ...
                        <button className="button" onClick={this.onFinished.bind(this, task.get('Id'))}>
                            <i className="fa fa-check"></i>Finish
                        </button>
                    </div>
                )
                    ...
                    <div className="small-12 columns">
                        <div className="section-header">
                            <div className="row expanded">
                                ...
                                <div className="small-2 column">{this.renderTaskDropdown()}</div>
                            </div>
                        </div>
                    </div>
                </div>
