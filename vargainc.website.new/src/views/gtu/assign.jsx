        ...
        return (
            <tr key={gtu.get('Id')}>
                ...
                <td className="text-center">
                    <input ref="userColor" className="color-block" type="color" autoComplete={true} defaultValue={'#' + Math.floor(Math.random() * 16777215).toString(16)} />
                </td>
                <td colSpan="2">
                        ...
                        {map(user, function (v, k) {
                            return (
                                <optgroup key={k} label={v[0].get('CompanyName')}>
                                    {v.map(function (u) {
                                        return (
                                            <option key={u.get('UserId')} value={u.get('UserId')}>
                ...
                <td className="text-center">
                    <button className="button tiny" onClick={self.onSave.bind(self, gtu.get('Id'))}>
                        Save
                    </button>
                    <button className="button tiny alert" onClick={self.onCancel}>
                        Cancel
                    </button>
                </td>
            </tr>
                    ...
                    <tbody>
                        {collection.map(function (gtu) {
                            var isAssign = gtu.get('IsAssign'),
                                gtuId = gtu.get('Id'),
                                assignButton = (
                                    <button className="button tiny" onClick={self.onAdd.bind(null, gtuId)}>
                                        <i className="fa fa-plus"></i>&nbsp;Assign
                                    </button>
                                ),
                                removeButton = (
                                    <button className="button alert tiny" onClick={self.onRemove.bind(null, gtuId)}>
