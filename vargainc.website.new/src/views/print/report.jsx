...
import $ from 'jquery'
import { some, cloneDeep, extend, pick, eq, each, omit, map, indexOf } from 'lodash'
    ...
    onPrint: function () {
        ...
        collecton.exportPdf(postData).then(function (response) {
            ...
            if ($('#downloadForm').size() == 0) {
                $('<form id="downloadForm" action="' + downloadUrl + '" method="GET"></form>')
                    .appendTo('body')
                    .get(0)
                    .submit()
            } else {
                ...
                <div className="row">
                    <div className="small-12 columns text-center">
                        <div className="button-group print-toolbar">
                            <button className="button" onClick={this.onOpenOptions}>
                                <i className="fa fa-cog"></i>Options
                            </button>
                            <button className="button" onClick={this.onPrint}>
                    ...
                    {pages.map(function (page) {
                        switch (page.get('type')) {
                            ...
                            case 'CampaignSummary':
                                return options.suppressCampaign || options.suppressCampaignSummary ? null : (
                                    <CampaignSummaryView ref={page.get('key')} key={page.get('key')} model={page} options={options} />
                                )
                            case 'SubMap':
                                return options.suppressSubMap ? null : <SubMapView ref={page.get('key')} key={page.get('key')} model={page} options={options} />
                                break
                            case 'SubMapDetail':
                                return options.suppressSubMap || options.suppressSubMapCountDetail ? null : (
                                    <SubMapDetailView ref={page.get('key')} key={page.get('key')} model={page} options={options} />
                                )
                                break
                            case 'DMap':
                                return options.suppressDMap || indexOf(hideDMaps, page.get('DMapId')) > -1 ? null : (
                                    <DMapView ref={page.get('key')} key={page.get('key')} model={page} options={options} />
                                )
                                break
                            case 'DMapDetailMap':
                                return options.suppressDMap || indexOf(hideDMaps, page.get('DMapId')) > -1 ? null : (
                                    <DMapDetailMapView ref={page.get('key')} key={page.get('key')} model={page} options={options} />
                                )
                                break
                            default:
                                return null
