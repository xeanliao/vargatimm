    ...
    if (area) {
        $.each(area, function () {
            if(this.Count > 0){
                ...
                $('<li class="hide" id="board-submap-' + this.Id + '"><span title="' + this.Name + '">' + this.Name + '</span><a class="button">' + this.Count + '</a></li>').appendTo(board);
            }
        });
    }
...
function OnPrintClick(version) {
    if (DMExits()) {
        ...
        //window.location = url;    
        window.open(url, '_blank', 'resizable=yes,status=yes,toolbar=no,menubar=no,location=no');
    } else
